; ---------------------------------------------------------------------------
; Cube Z80 Sound Driver
;
; Disassembled by Wizardwhosaysni(02.08.2014).
; https://github.com/wizardwhosaysni/CUBEWIZ
; ---------------------------------------------------------------------------
...
; Constants
SFX_BANK 							equ 00h
SFX_BANK_OFFSET 					equ 08000h

DAC_BANK_1 						equ (1D0000h>>15)

MUSIC_BANK_1 						equ (1D8000h>>15)
MUSIC_BANK_2 						equ (1E0000h>>15)
MUSIC_BANK_3 						equ (1E8000h>>15)

YM_INSTRUMENTS_BANK 			equ (1C8000h>>15)		; 1E8000h|E000h
YM_INSTRUMENTS_BANK_OFFSET	equ 0CA00h				; “екущий офсет(не плюсовать). 0 = 8000h
; ---------------------------------------------------------------------------
...
loc_6B:					; Conditional input to implement here !
		call	YM_Input
		inc	b
		call	YM_Input
		inc	b
...
loc_201:
		cp	01Eh ; '!'
		jr	nc, loc_201_1	; if a > 21h, then play	music from chunk 0x1F0000
		ld	a, MUSIC_BANK_2
		ld	(MUSIC_BANK), a
...
Activate_Stereo_Outputs:
		...
		ld	a, 0C0h	; 'ј'   ; set C0h for byte 1E of YM channels
		...
		ld	a, 0FEh	; 'ю'   ; put value FE as DAC Sound sample to load ... which loads nothing. Why such a thing then ?
...
loc_309:
		...
		ld	b, a
		ld	c, 0C0h	; 'ј'   ; sets stereo left and right on
		call	YM2_Input
...
PSG_SetChannelAttenuation:		; CODE XREF: PSG_ParseToneData+4Fj
		...
		or	90h ; 'Р'       ; load attenuation command bits
...
YM1_ParseData:				; CODE XREF: UpdateSound+70p
		...
		or	a
		jr	z, loc_nimp
		ld	de, 01E0h ; 'а'  ; point to the right SFX channel data
		add	ix, de
loc_nimp:
		pop	af
		ld	c, 0A0h	; '†'
...
loc_5D7:				; CODE XREF: YM1_ParseData+6Bj
		...
		add	ix, bc
		ld	a, 0B4h	; 'і'   ; YM Register : Stereo / LFO Sensitivity
		add	a, (iy+0)
		ld	b, a
...
Set_Instrument:				; CODE XREF: YM1_ParseData+5Dj
		cp	0FEh ; 'ю'      ; FE xx : set new instrument xx
...
Load_Note:				; CODE XREF: YM1_ParseData+7Ej
		cp	0FDh ; 'э'      ; FD xx : play note at level xx. It's followed by other parameters handled at next parsing loop
...
YM2_ParseChannel6Data:			; CODE XREF: UpdateSound+94p
		...
		or	a
		jr	z, loc_B68
		ld	de, 01E0h ; 'а'  ; if we are currently managing an SFX using the 3 extra RAM areas, point to the right area
		add	ix, de
...
Transmit_Attenuation:			; CODE XREF: PSG_ParseNoiseData+111j
		...
		or	0F0h ; 'р'      ; add attenuation command bits
...
t_SAMPLE_LOAD_DATA: 		; Data layout of the 8-byte entries
		db 2, 0,		DAC_BANK_1, 0, 51h, 9, 0, 80h			; byte 0 : PCM frame period parameter
		db 2, 0,		DAC_BANK_1, 0, 0Ah, 0Fh, 52h, 89h	; byte 1 : always 0 (ignored when data is parsed)
		db 1, 0,		DAC_BANK_1, 0, 0B3h, 13h, 5Dh, 98h	; byte 2 : bank to load
		db 4, 0,		DAC_BANK_1, 0, 0B3h, 13h, 5Dh, 98h	; byte 3 : always 0 (ignored when data is parsed)
		db 8, 0, 		DAC_BANK_1, 0, 0B3h, 13h, 5Dh, 98h	; bytes 5-4 : sample length
		db 0Fh, 0,	DAC_BANK_1, 0, 17h, 1Eh, 11h,	0ACh	; bytes 7-6 : pointer to sound PCM Data once bank is loaded
		db 1, 0, 		DAC_BANK_1, 0, 5, 1Eh, 29h, 0CAh
		db 3, 0, 		DAC_BANK_1, 0, 5, 1Eh, 29h, 0CAh		; With this table, the same sound sample can be played at different rates,
		db 4, 0, 		DAC_BANK_1, 0, 5, 1Eh, 29h, 0CAh		; resulting with quick high-pitched, or slow low-pitched sounds,
		db 6, 0, 		DAC_BANK_1, 0, 5, 1Eh, 29h, 0CAh		; which is quite appropriate	for drums or attack hits or explosions !
		db 8, 0, 		DAC_BANK_1, 0, 5, 1Eh, 29h, 0CAh
		db 1, 0, 		DAC_BANK_1, 0, 0C3h, 1Dh, 0D5h,	87h
		db 2, 0,		DAC_BANK_1, 0, 0C3h, 1Dh, 0D5h,	87h
		db 5, 0,		DAC_BANK_1, 0, 0C3h, 1Dh, 0D5h,	87h
		db 6, 0,		DAC_BANK_1, 0, 0C3h, 1Dh, 0D5h,	87h
		db 5, 0,		DAC_BANK_1, 0, 0B2h, 2Ch, 99h, 0A5h
		db 1, 0, 		DAC_BANK_1, 0, 50h, 18h, 4Ch, 0D2h
		db 1, 0,		DAC_BANK_1, 0, 93h, 1Ch, 0, 80h
		db 1, 0, 		DAC_BANK_1, 0, 0AAh, 19h, 94h, 9Ch
		db 5, 0, 		DAC_BANK_1, 0, 50h, 18h, 4Ch, 0D2h
		db 0Fh,	0,	DAC_BANK_1, 0, 0EBh, 18h, 3Fh, 0B6h
		db 0Fh,	0,	DAC_BANK_1, 0, 0C0h, 1Dh, 0, 80h
		db 6, 0, 		DAC_BANK_1, 0, 0Dh, 1Ah, 2Bh, 0CFh
		db 0Ah,	0, 	DAC_BANK_1, 0, 2Fh, 1Dh, 0, 80h
		db 6, 0,		DAC_BANK_1, 0, 5Dh, 10h, 9Dh, 0EAh
		db 6, 0,		DAC_BANK_1, 0, 7Bh, 0Fh, 39h, 0E9h
		db 2, 0,		DAC_BANK_1, 0, 0FCh, 20h, 30h, 9Dh
		db 8, 0,		DAC_BANK_1, 0, 0FCh, 20h, 30h, 9Dh
		db 0Fh, 0, 	DAC_BANK_1, 0, 0FCh, 20h, 30h, 9Dh
		db 1, 0,		DAC_BANK_1, 0, 69h, 7, 2Fh, 0E8h
		db 1Eh, 0, 	DAC_BANK_1, 0, 17h, 1Eh, 11h,	0ACh
		db 3, 0,		DAC_BANK_1, 0, 0D4h, 7,	0, 80h
		db 8, 0,		DAC_BANK_1, 0, 0C0h, 1Dh, 0, 80h
		db 5, 0,		DAC_BANK_1, 0, 0D3h, 1Fh, 2Dh, 0BEh
		db 2, 0,		DAC_BANK_1, 0, 0Ah, 0Fh, 99h, 0EFh
		db 8, 0,		DAC_BANK_1, 0, 7, 1Ch, 1, 0DEh
		db 3, 0,		DAC_BANK_1, 0, 10h, 50h, 0C1h, 9Dh

...
Console	macro	argument1, argument2
	...
	case "write"
		...
		lea		4*4(sp), a2
		lea		__data(pc), a1
		jsr	(ErrorHandler___global__console_write_formatted).l
		movem.l	(sp)+, a0-a2/d7
		if (__sp>8)
			lea		__sp(sp), sp
		elseif (__sp>0)
	...
	case "writeline"
		...
		lea		4*4(sp), a2
		lea		__data(pc), a1
		jsr	(ErrorHandler___global__console_writeline_formatted).l
		movem.l	(sp)+, a0-a2/d7
		if (__sp>8)
			lea		__sp(sp), sp
		elseif (__sp>0)
	...
	case "run"
		jsr	(ErrorHandler___extern__console_only).l
		jsr	(argument1).l
		bra.s	*
	...
	case "setxy"
		...
		move.w	argument2, -(sp)
		move.w	argument1, -(sp)
		jsr	(ErrorHandler___global__console_setposasxy_stack).l
		addq.w	#4, sp
		movem.l	(sp)+, d0-d1
		move.w	(sp)+, sr
	...
	case "breakline"
		move.w	sr, -(sp)
		jsr	(ErrorHandler___global__console_startnewline).l
		move.w	(sp)+, sr
